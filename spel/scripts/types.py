...
from spel.scripts.fortran_parser.spel_ast import BlockStatement, Expression, IfConstruct
...
class ParseState:
    ...
    path: Path  # path to original file
...
class VariableAccess:
    """
    Metadata for variable access at a line
    """

    lineno: int
    mode: ReadWrite
    guard: ConditionExpectation = NO_EXPECTATION


@dataclass
class VariableAccessTable:
    by_variable: dict[str, list[VariableAccess]] = field(default_factory=dict)

    def get(self, name: str) -> Optional[list[VariableAccess]]:
        return self.by_variable.get(name)
